...
- **形态**：以插件来源（`source.kind === 'plugin'`）的 user 角色消息追加到会话的 surface
  （`surfaceOp: 'append'`），使 agent loop 在重发时可见。追加失败只记警告，不影响续跑。
- **角色**：user 角色，**不使用** `system/message`（决策与依据见 §6.1）。

### 6.1 为什么不用 `system/message`（调查结论，基于 DSH 0.1.5 源码）

`system/message` 在 DSH 中是**渲染后的系统提示词**所在的 surface 节点，不是「插件通知通道」：

1. 系统提示词的组装方（`dsh-agent-loop` 的 `SystemPromptProjection`）把**所有**存活的
   `system/message` 节点都纳入自己的管理；当路由不支持 in-history 更新、或新的消息系列开始、
   或本次渲染为空时，它会把**所有非首节点置空**——被置空的节点不产生任何 wire message。
2. 是否支持 in-history 由模型路由声明（`systemPromptUpdate === 'in-history'`）。DSH `0.1.5` 中
   **只有 DeepSeek 路由声明了它**；OpenAI 兼容路由（含 ZenMux 所走的 pi-ai）都没有声明。
   因此本插件若追加 `system/message`，**在目标平台上会被静默置空，通知根本到不了模型**；
   而在支持 in-history 的路由上，它会插进系统提示词节点序列，客户端轨迹界面还会把它显示成
   「系统提示词已更新」，语义同样不符。
3. 框架对「插件注入的合成上下文」给出的唯一公开入口是 user 角色
   （`Agent.inject(input: UserMessage)`）；一方插件（`agent-instructions` 注入 AGENTS.md 与
   文件变更通知、`tool-skill` 注入技能内容）也都采用 **user 角色 + 调用方自带的
   `<system-reminder>` 包裹**来表达「这不是用户发言」。

结论：提示消息保持 user 角色 + plugin 来源（框架与界面据此即可区分它不是用户输入）。
若将来需要更强的「系统级」观感，正确做法是把说明包进 `<system-reminder>`（与一方插件一致），
**而不是**借用 `system/message`。本决策依赖上述框架语义，升级 DSH 后应重新核对。
