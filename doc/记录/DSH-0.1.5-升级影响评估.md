...
1. ~~**真实 402 黄金样本仍未核对**~~ → **已核对，无残留**（见 `PROGRESS.md`「黄金样本回归」样本 2/3）：
   账号达到配额上限时的真实统计响应（覆盖「5h 已耗尽」分支）与**线上抓取的真实 402 错误响应体**
   均已逐字节入库；402 识别规则按真实响应体回归通过（压平形态用真实 `dsh-llm-pi-ai`
   `formatProviderError` 实测复现），现有规则无需修订。
